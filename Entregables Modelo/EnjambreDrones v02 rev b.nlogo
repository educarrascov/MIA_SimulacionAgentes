...
turtles-own [
  ...
  prioridad          ; solo para tipos objeto y drone, contiene orden de la ruta
  estado             ; Para tipo drone y objeto
  distancia          ; solo para tipo objetcos
]
    ...
    ask turtle id-destino [
      ...
      ifelse (F-rango round(tmp-xcor-drone) round(tmp-ycor-drone) xcor ycor 2)[  ; Drone llegó al objeto ?
        ...
        [ ; else, implica que encontró un objeto
          ...
          if (color = red)[ ; Es nocivo ?
                            ; lo marcamos con un círculo rojo
            F-dibuja-circulo xcor ycor q-size-drone color 1 true
          ]
          ...
          ; buscar nuevo objetivo, ajustar rutas y cakibrar direcciones
...
to setup_globals
  ...
  set k-lejania-borde 10
  set k-lejanía-torre-control 30
...
to setup_turtles
  ...
  ;; Objetos NO nocivos
  let rec 0
  create-turtles q-objetos-NO-nocivos [
    set tipo "object"
  ...
  ;; Objetos nocivos
  set rec 0
  create-turtles q-objetos-nocivos [
    set tipo "object"
    set subtipo "nocive"
    set id-drone-asignado -1
    set prioridad -1
    set color red
    set size q-size-objeto + 1
    move-to one-of patches
    ; Ubicarlo en posición permitida
    while [not F-coordenada-permitida xcor ycor][
      ;set xcor random max-pxcor * 2 - max-pxcor
  ...
  create-turtles q-drones [
    ...
    set size q-size-drone
    set xcor q-drones / -2 * q-size-drone + (who + 1 - q-old-turtles - q-drones + 0.5) * q-size-drone
    set ycor q-size-drone * -1.5
    set shape "drone4"

    ; primera asignación
    set id-origen (who - q-drones) ; El initial correspondiente a cada drone, el destino se determinará por distancia
    set prioridad -1 ; Suma 1 en la primera pasada
    set estado "en-ruta"
    ; crea link de comunicaciones
    F-crea-link who 0 gray + 3 0.2
  ]
...
to-report F-direction [x1 y1 x2 y2]
  let dir-direction -1
  ; todo: calcular la dirección en casos en que y1 = y2 o x2 = x1
  ifelse (x1 = x2)[
    ifelse (y1 < y2)[
      set dir-direction 0
    ]
...
;; Creación de links
;;
to F-crea-link [lnk-id-origen lnk-id-destino lnk-color lnk-thinkness]
...
to F-asigna-mas-cercano-OBJ
  ...
  ask turtles with [tipo = "drone" and subtipo = "inspector"][
    ...
    ask turtles with [tipo = "object" and estado = "non-inspected" and id-drone-asignado = tmp-id-drone][
    ...
    ifelse (b-entro)[
      ...
      let tmp-xcor-destino -1
      let tmp-ycor-destino -1
      ask turtles with [who = id-destino][
        set tmp-xcor-destino xcor
        set tmp-ycor-destino ycor
      ]
